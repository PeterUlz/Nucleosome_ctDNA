...
def thread_proc(q,thread_number,transcript_list):
    ...
    for transcript in transcript_list:
        ...
        temp_filename= args.temp_dir+os.path.basename(args.bigwig_file)+str(thread_number)+"tmp_coverage.bed"
        TMP_COVERAGE_BED = open(temp_filename,"w")
        call(["scripts/bigWigToWig","-chrom="+chrom,"-start="+str(pos-args.start),"-end="+str(pos+args.end+1),args.bigwig_file,temp_filename])
        TMP_COVERAGE_BED.close()
    
        TMP_COVERAGE_BED_OUTPUT = open(temp_filename,"r")
        header = TMP_COVERAGE_BED_OUTPUT.readline()
        content = TMP_COVERAGE_BED_OUTPUT.readlines()
        for line in content:
            if line[:1] == "#":
                continue
            info = line.split("\t")
            for bigwig_pos in range(int(info[1]),int(info[2])):
                if bigwig_pos >= pos-args.start and bigwig_pos < pos+args.end+1:
                    if forward:
                        position_lists[bigwig_pos-pos].append(float(info[3]))
                    else:
                        position_lists[-(bigwig_pos-pos)].append(float(info[3]))
        TMP_COVERAGE_BED_OUTPUT.close()
        call(["rm",temp_filename])    
...
parser.add_argument('-bw','--bigwig', dest='bigwig_file',
                   help='BigWig file',required=True)
...
sys.stderr.write("BigWig file: "+args.bigwig_file+"\n")
